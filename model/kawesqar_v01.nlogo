...
to create-agents
  create-people n-people [
    set whale-hunt random-float 1
    set help-others random-float 1
...
to take-decisions
  ask people [
    set whaling? ifelse-value (random-float 1 < whale-hunt) [True] [False]
    set helping? ifelse-value (random-float 1 < help-others) [True] [False]
  ]
end

to whaling-harm
  ask people with [whaling? = True] [
    set disabled? ifelse-value (random-float 1 < whaling-harm-prob) [True] [False]
  ]
...
to get-social-capital
  ask people [
   ...
   (ifelse (
             ; major cooperators
             (helping? = True) and (whaling? = True))
                  [ set social-capital truncated-normal mu-high ]
             ; intermidiate cooperators
             ( (helping? = True) and (whaling? = False))
                  [ set social-capital truncated-normal mu-medium ]
             ( (helping? = False) and (whaling? = True))
                  [ set social-capital truncated-normal mu-medium ]
             ; defectors
                  [ set social-capital truncated-normal mu-low ]
   )
  ]
...
to get-fitness
  ask people [
    set fitness (resources ^ theta) * (social-capital ^ (1 - theta))
  ]
end
...
to imitate-strategies
  ...
  ; random tournament
  let agentlist shuffle [who] of people

  while [not empty? agentlist] [

   let a1 first agentlist
   set agentlist but-first agentlist

   if not empty? agentlist [

      let a2 first agentlist
      set agentlist but-first agentlist

      ; Imitation
      ifelse ([fitness] of person a1) < ([fitness] of person a2)
      [ ;; a1 copy a2
        ask person a1 [ set help-others [help-others] of person a2
                        set whale-hunt [whale-hunt] of person a2 ] ]
      [
        ;; a2 copy a1
        ask person a2 [ set help-others [help-others] of person a1
                        set whale-hunt [whale-hunt] of person a1 ] ]
    ]
  ]
...
to update-position
  set size 0.5
  set color 19 - (fitness * 4)
  setxy (whale-hunt * max-pxcor) ( help-others * max-pycor)
  rt random 360
  fd random-float 2
end
...
to-report logistic-whaling-success-prob [x scale]
  ;; Logistic function
  report 1 / (1 + exp( - (x - mu-logistic-whaling-success) / scale))
end

to-report truncated-normal [mu]
  let x random-normal mu 0.1
  set x min list x 1
  set x max list 0 x
  report x
end
...
"" ""
PENS
"default" 2.0 1 -8053223 true "" "histogram sort [100 * precision whale-hunt 2] of people"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot average-disabled"
...
"" ""
PENS
"default" 2.0 1 -16777216 true "" "histogram sort [100 * precision help-others 2] of people"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot average-social-capital"
"pen-1" 1.0 2 -2674135 true "" "plot mu-low"
"pen-2" 1.0 2 -13345367 true "" "plot mu-high"
